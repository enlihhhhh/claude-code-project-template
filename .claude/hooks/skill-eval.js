...
function extractFilePaths(prompt) {
  ...
  // Match explicit paths with Python/research extensions
  const extensionPattern =
    /(?:^|\s|["'`])([\w\-./]+\.(?:py|ipynb|yaml|yml|json|toml|cfg|sh|pt|ckpt|csv|tsv|txt|md))\b/gi;
  let match;
  while ((match = extensionPattern.exec(prompt)) !== null) {
    paths.add(match[1]);
  }

  // Match paths starting with common research directories
  const dirPattern =
    /(?:^|\s|["'`])((?:src|experiments|notebooks|scripts|configs|data|results|tests|models|checkpoints|outputs|\.claude|\.github)\/[\w\-./]+)/gi;
