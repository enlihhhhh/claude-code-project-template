...
Run `scripts/fetch_paper.py`. The script tries sources in order of quality:
1. **LaTeX source** from `https://arxiv.org/src/{id}` (preferred — exact equations, no parsing artifacts, full document structure). Recursively resolves `\input{}` and `\include{}` directives.
2. **PDF extraction** via `pymupdf4llm` (preserves math notation as LaTeX)
3. **PDF extraction** via `pdfplumber` if pymupdf4llm fails
4. **HTML** from `https://ar5iv.labs.arxiv.org/html/{id}` if PDF extraction produces garbled output
...
### If LaTeX source download fails:
1. The script automatically falls back to PDF extraction
2. LaTeX source may not be available for non-arxiv papers or very old papers
