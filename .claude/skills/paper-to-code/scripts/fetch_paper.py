...
def download_latex_source(arxiv_id: str, output_dir: Path) -> str | None:
    """Download and extract LaTeX source from arxiv e-print.

    This is the preferred method — LaTeX source preserves exact equations,
    document structure, and avoids all PDF parsing artifacts.
    Inspired by Karpathy's read-arxiv-paper approach.
    """
    base_id = re.sub(r"v\d+$", "", arxiv_id)
    src_url = f"https://arxiv.org/src/{arxiv_id}"
    src_dir = output_dir / "latex_source"
    ...
    try:
        ...
        tarball_path = output_dir / "source.tar.gz"
        ...
        if file_size < 100:
            print("  WARNING: Source file too small, likely not a valid archive.", file=sys.stderr)
            return None
        ...
        try:
            ...
            # Some sources are just a single .tex file (not gzipped)
            try:
                with tarfile.open(tarball_path, "r:") as tar:
                    tar.extractall(path=src_dir, filter="data")
            except (tarfile.TarError, Exception):
                # Might be a single uncompressed .tex file
                import gzip

                try:
                    with gzip.open(tarball_path, "rt", encoding="utf-8", errors="replace") as gz:
                        tex_content = gz.read()
                    single_tex = src_dir / "main.tex"
                    single_tex.write_text(tex_content, encoding="utf-8")
                except Exception:
                    # Last resort: treat as plain text
                    try:
                        tex_content = tarball_path.read_text(encoding="utf-8", errors="replace")
                        if "\\begin{document}" in tex_content or "\\documentclass" in tex_content:
                            single_tex = src_dir / "main.tex"
                            single_tex.write_text(tex_content, encoding="utf-8")
                        else:
                            print("  WARNING: Could not extract LaTeX source.", file=sys.stderr)
                            return None
                    except Exception:
                        print("  WARNING: Could not extract LaTeX source.", file=sys.stderr)
                        return None
        ...
        if not tex_files:
            print("  WARNING: No .tex files found in source.", file=sys.stderr)
            return None

        # Heuristic: main file is the one with \documentclass or \begin{document}
        main_tex = None
        for tf in tex_files:
            content = tf.read_text(encoding="utf-8", errors="replace")
            if "\\begin{document}" in content or "\\documentclass" in content:
                main_tex = tf
                break
        ...
        print(f"  Found {len(tex_files)} .tex file(s), main: {main_tex.name}")

        # Read all tex files, following \input{} and \include{} directives
        full_text = _resolve_latex_includes(main_tex, src_dir)

        if len(full_text) > 500:
            print(f"  Extracted: {len(full_text)} characters from LaTeX source")
            return full_text

        print("  WARNING: LaTeX source produced insufficient text.", file=sys.stderr)
        return None

    except requests.RequestException as e:
        print(f"  LaTeX source download failed: {e}", file=sys.stderr)
        return None


def _resolve_latex_includes(tex_file: Path, root_dir: Path, depth: int = 0) -> str:
    """Recursively resolve \\input{} and \\include{} directives in LaTeX source."""
    if depth > 10:
        return ""
    ...
    def replace_include(match: re.Match) -> str:
        ...
        # Search relative to root and relative to current file
        candidates = [
            root_dir / filename,
            tex_file.parent / filename,
        ]
        for candidate in candidates:
            if candidate.exists():
                return _resolve_latex_includes(candidate, root_dir, depth + 1)
...
def main():
    ...
    # Step 3: Try LaTeX source first (best quality — exact equations, no parsing artifacts)
    paper_text = None
    extraction_method = None

    print("\n--- Trying LaTeX source (preferred) ---")
    paper_text = download_latex_source(arxiv_id, output_dir)
    if paper_text:
        extraction_method = "latex_source"

    # Step 4: Fall back to PDF extraction
    ...
    # Step 5: Fall back to ar5iv HTML
    if paper_text is None:
    ...
    # Step 7: Search for official code repositories
    ...
    method_labels = {
        "latex_source": "LaTeX source (best quality)",
        "pdf": "PDF extraction",
        "ar5iv_html": "ar5iv HTML",
    }

    print(f"\n--- Extraction Summary ---")
    print(f"  Output: {text_path}")
    print(f"  Method: {method_labels.get(extraction_method, extraction_method)}")
    print(f"  Characters: {len(paper_text):,}")
    print(f"  Pages detected: {page_count if page_count > 0 else 'N/A'}")
